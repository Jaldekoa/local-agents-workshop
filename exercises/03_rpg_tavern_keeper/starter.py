"""Exercise 03 — Grunk, the haggling tavern keeper. 100% local.

The lesson: STATE LIVES IN CODE, personality lives in the model.
Prices, stock and your gold are plain Python (inventory.py) — exact and
un-hallucinatable. Grunk's grumpy charm is the model — fuzzy and fun.
The tools are the border between the two worlds.

Your three TODOs:
  #1  implement the haggling rule inside make_offer
  #2  wire the three tools to the model
  #3  write Grunk's personality (the creative one — see the README template)

Run from this folder (inventory.py must be importable):
...
def make_offer(item: str, gold: int) -> str:
    ...
    # TODO(you) #1 — THE HAGGLING RULE. Plain arithmetic no prompt can override:
    #
    #   offer >= 80% of base   -> ACCEPT: found["stock"] -= 1,
    #                             player["gold"] -= gold, return a "DEAL!"
    #                             string that includes the remaining gold/stock
    #   offer >= 50% of base   -> COUNTEROFFER: don't touch any state; return a
    #                             grumbling string proposing the midpoint,
    #                             e.g. counter = round((gold + base) / 2)
    #   anything lower         -> REFUSE, colorfully. Mention the asking price
...
# TODO(you) #3 — write Grunk! The README has a fill-in-the-blanks template.
# Two hard-won rules for prompting a 293M model (we learned these testing):
#   - Keep it SHORT and imperative, and NAME the tools: "FIRST call a tool:
#     list_wares to..., ask_price for..., make_offer when...". Vague rules
...
llm = ChatOllama(
    ...
    # cap). With it, a runaway turn costs seconds, the reply comes back
